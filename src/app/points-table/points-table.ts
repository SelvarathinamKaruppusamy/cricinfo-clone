  ...
  loadPointsTable(): void {
    this.completedService.getCompletedMatches().subscribe({
      next: (matches: any[]) => {
        ...
        if (matchNoParam) {
          const selectedMatchNo = Number(matchNoParam);

          filteredMatches = matches.filter(
            (match) => match.matchNo <= selectedMatchNo
          );
        }
        ...
        // Update stats only for matches included
        ...
        this.pointsTable.forEach((team) => {
          const runRateFor =
            team.ballsFaced > 0
              ? team.runsScored / (team.ballsFaced / 6)
              : 0;

          const runRateAgainst =
            team.ballsBowled > 0
              ? team.runsConceded / (team.ballsBowled / 6)
              : 0;
